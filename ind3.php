...
th{
	...
	<body style="background-image: url(images/backgroundimg.jpeg); background-repeat: cover;">
		...
		<?php
		 ...
		 $result = $mysqli->query("SELECT * FROM hungerspot") or die(mysqli_error);
		 //pre_r($result);
		 ?>
		 ...
		 <div class="row justify-content-center">
		   ...
		   <?php
		 		while ($row = $result->fetch_assoc()): ?>
		 			<tr>
		 				<td><?php echo $row['hspot']; ?></td>
		 				<td><?php echo $row['details']; ?></td>
		 				<td><?php echo $row['type']; ?></td>
		 				<td><?php echo $row['quantity']; ?></td>
		 				<td><?php echo $row['status']; ?></td>
		 				<td>
		 					<a href="ind3.php?edit=<?php echo $row['id']; ?>"
		 						class="btn btn-info" style="font-size: 20px;">Edit</a>
		 						<a href="pro3.php?delete=<?php echo $row['id']; ?>"
		 							class="btn btn-danger" style="font-size: 20px;">Delete</a>
		 				</td>
		 			</tr>
		 		<?php endwhile;?>
		 	</table>
		...
		<form action="pro3.php" method="POST">
			...
			<label>Hungerspot</label>
			<input type="text" name="hspot" class="form-control" value="<?php echo $hspot; ?>" placeholder="enter the name">
			</div>
			<div class="form-group">
			<label>Details</label>
...
<button class="button button1" id="logout"><a href="moh.php">BACK</a></button>
